#include <iostream>
using ll = long long;
using namespace std;

const int maxN = 2e5 + 5;

int N, Q;
int a[maxN];

struct node {
  ll val;
  ll lzAdd;
  ll lzSet;
  node(){};
} tree[maxN << 2];

#define lc p << 1
#define rc (p << 1) + 1

// Why are we only pushing down when needed? Time Complexity?

inline void pushup(int p) { tree[p].val = tree[lc].val + tree[rc].val; }

// can we avoid setting lzAdd = 0 when we set a new value to lzSet
void pushdown(int p, int l, int mid, int r) {
  if (tree[p].lzSet != 0) {
    tree[lc].lzSet = tree[p].lzSet;
    tree[rc].lzSet = tree[p].lzSet;
    tree[lc].val = (mid - l + 1) * tree[p].lzSet;
    tree[rc].val = (r - mid) * tree[p].lzSet;
    tree[lc].lzAdd = tree[rc].lzAdd = 0;
    tree[p].lzSet = 0;
  } else if (tree[p].lzAdd != 0) {
    if (tree[lc].lzSet != 0) {
      tree[lc].lzSet += tree[p].lzAdd;
      tree[lc].lzAdd = 0;
    } else {
      tree[lc].lzAdd += tree[p].lzAdd;
    }

    if (tree[rc].lzSet != 0) {
      tree[rc].lzSet += tree[p].lzAdd;
      tree[rc].lzAdd = 0;
    } else {
      tree[rc].lzAdd += tree[p].lzAdd;
    }

    tree[lc].val += (mid - l + 1) * tree[p].lzAdd;
    tree[rc].val += (r - mid) * tree[p].lzAdd;
    tree[p].lzAdd = 0;
  }
  return;
}

// build tree
void build(int p, int l, int r) {
  if (l == r) {
    tree[p].val = a[l];
    return;
  }
  int mid = (l + r) / 2;
  build(lc, l, mid);
  build(rc, mid + 1, r);
  pushup(p);
}

void add(int p, int l, int r, int a, int b, ll val) {
  if (b < l || a > r) {
    return;
  } else if (a <= l && r <= b) {
    tree[p].val += (r - l + 1) * val;
    if (tree[p].lzSet == 0) {
      tree[p].lzSet += val;
    } else {
      tree[p].lzAdd += val;
    }
    return;
  }

  int mid = (l + r) / 2;
  // we call pushdown here because we are about to enter the left/right child
  // we need to deliver the correct information to our children
  // we rely on this information within the logic above when a <= l && r <= b
  pushdown(p, l, mid, r);
  add(lc, l, mid, a, b, val);
  add(rc, mid + 1, r, a, b, val);
  // pushup needed when affecting a gn-child and need to build back up
  pushup(p);
  return;
}

void set(int p, int l, int r, int a, int b, ll val) {
  if (b < l || r < a) {
    return;
  } else if (a <= l && r <= b) {
    tree[p].val = (r - l + 1) * val;
    tree[p].lzSet = val;
    tree[p].lzAdd = 0;
    return;
  }

  int mid = (l + r) / 2;
  pushdown(p, l, mid, r);
  set(lc, l, mid, a, b, val);
  set(rc, mid + 1, r, a, b, val);
  pushup(p);

  return;
}

ll query(int p, int l, int r, int a, int b) {
  if (b < l || r < a) {
    return 0;
  }
  if (a <= l && r <= b) {
    return tree[p].val;
  }

  int mid = (l + r) / 2;
  pushdown(p, l, mid, r);
  return query(lc, l, mid, a, b) + query(rc, mid + 1, r, a, b);
}

signed main() {
  ios_base::sync_with_stdio(false);
  cin.tie(0);
  cin >> N >> Q;
  for (int i = 1; i <= N; ++i)
    cin >> a[i];

  build(1, 1, N);
  for (int i = 1; i <= Q; ++i) {
    int x;
    cin >> x;
    if (x == 1) {
      int y, z, val;
      cin >> y >> z >> val;

      add(1, 1, N, y, z, val);
    } else if (x == 2) {
      int y, z, val;
      cin >> y >> z >> val;

      set(1, 1, N, y, z, val);
    } else {
      int y, z;
      cin >> y >> z;

      cout << query(1, 1, N, y, z) << endl;
    }
  }

  return 0;
}